            ...
            if (hasMogera && mogera->getalive() && !mogera->isDying()) {
                for (int c = 0; c < Mogera::MAX_CHILDREN; c++) {
                    ...
                    if (sbHB.findIntersection(mogera->children[c].getHitbox())) {
                        mogera->children[c].alive = false;
                        sb->setalive(false);
                        scoreSystem.onBottomKilled();   // small enemy score
                        gemSystem.enemykilled();
                        spawnGem(mogera->children[c].x, mogera->children[c].y);
                        break;
                    }
                }
            }
...
// File naming convention: assets\images\lvl1.png, lvl2.png, etc.
        ...
        if (lvl.isBosslevel()) {
            if (lvl.getLevelno() == 5) {
                // Bottom-right corner: x=500 leaves 280px for boss body, y=360 sits on floor
                mogera = new Mogera(500, 360);
