...
struct OrbBomb {
    ...
    static constexpr float FLASH_START = 1.5f; 
    ...
    FloatRect getHitbox() const { return FloatRect(sf::Vector2f((float)x, (float)y), Vector2f(W, H)); }
    FloatRect getBlastBox() const { return sf::FloatRect(Vector2f((float)x - EXPLODE_RADIUS, (float)y - EXPLODE_RADIUS),Vector2f(W + EXPLODE_RADIUS * 2.f, H + EXPLODE_RADIUS * 2.f)); }
...
public:
    static const int MAX_ROCKETS = 1; 
    static const int MAX_CHILDREN = 1; 
    static const int MAX_BOMBS = 10;

    struct RocketStub {
        bool alive = false;
        FloatRect getHitbox() const { return {}; }
    };
    struct ChildStub {
        bool alive = false;
        FloatRect getHitbox() const { return {}; }
    ...
    FloatRect getHitbox() const {return FloatRect( Vector2f(HEAD_X, HEAD_Y), Vector2f(DISPLAY_W, DISPLAY_H)); }
...
private:
    ...
    static constexpr float HEAD_Y = 110.f; 
    ...
    static constexpr float FIRE_INTERVAL = 0.0f; 
    ...
    void draw(sf::RenderWindow& window) override {
        ...
        for (int i = 0; i < MAX_BOMBS; i++) {
            ...
            if (bombs[i].exploding) {
                ...
                blast.setPosition({(float)bombs[i].x + OrbBomb::W * 0.5f, (float)bombs[i].y + OrbBomb::H * 0.5f});
                blast.setFillColor(Color(255, 140, 0, 80));
                blast.setOutlineColor(Color(255, 200, 0, 180));
        ...
        for (int i = 0; i < MAX_FOAM; i++) {
            ...
            if (textureLoaded && foamSprite) {
                IntRect r = foamFrameFor(foamWaves[i].animFrame);
                foamSprite->setTextureRect(r);
                foamSprite->setScale({  FoamWave::W / static_cast<float>(r.size.x),
                ...
                RectangleShape r(Vector2f(FoamWave::W, FoamWave::H));
                r.setFillColor(Color(200, 240, 255, 180));
        ...
        for (int i = 0; i < MAX_EXPLOSIONS; i++) {
            ...
            if (textureLoaded && explosionSprite) {
               IntRect r = (explosions[i].frame == 0) ?  explosion_1 : explosion_2;
                explosionSprite->setTextureRect(r);
                float sz = explosions[i].isBombBlast ? 100.f : 70.f;
                explosionSprite->setScale({  sz / static_cast<float>(r.size.x),  sz / static_cast<float>(r.size.y) });
        ...
        if (textureLoaded && headSprite) {
            IntRect hr = mouthOpen ? head_open : head_closed;
            headSprite->setTextureRect(hr);
            headSprite->setScale({DISPLAY_W / static_cast<float>(hr.size.x),   DISPLAY_H / static_cast<float>(hr.size.y)  });
            headSprite->setPosition({ HEAD_X, HEAD_Y });
            if (dying && !flashVis)
                headSprite->setColor(Color(255, 80, 80, 255));
            else
            ...
            RectangleShape r(Vector2f(DISPLAY_W, DISPLAY_H));
            r.setPosition({ HEAD_X, HEAD_Y });
            r.setFillColor(dying && !flashVis ?
                Color(255, 80, 80) : Color(180, 50, 50));
            window.draw(r);
        }

        
        if (eyeVisible && textureLoaded && eyeSprite) {
            IntRect er = eyeFrameFor(eyeFrame);
            eyeSprite->setTextureRect(er);
            eyeSprite->setScale({ 80.f / static_cast<float>(er.size.x), 80.f / static_cast<float>(er.size.y) });
            eyeSprite->setPosition({ HEAD_X + DISPLAY_W * 0.5f - 40.f,HEAD_Y + DISPLAY_H * 0.35f - 40.f });
